...
**Domain ownership check:** as of 2026-07-02, `chief-of-staff.js` automatically scans for this every run (`findContentOverrideRisks()`) and surfaces candidates in the standup under "Content Override Risks" — this is chief's job, not a manual Big C checklist item (corrected after the earlier "manual only" call turned out wrong — see `project_bsv_chief_governance_gap` memory). Still worth a manual glance too when you touch copy, images, scheduling, or a pipeline script directly — the automated scan is a heuristic (flags candidates, doesn't judge them), not a substitute for actually looking at what you changed. If you find a real conflict, fix it at the owning agent and note it in the audit log.
