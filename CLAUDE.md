...
Before responding to any message in any session, read **both**:
1. `BSV-Memory.md` — the brand story, voice, strategic decisions, and open questions.
2. `BSV-BigC-Audit-Log.md` — the running, chronological play-by-play of what Big D and Big C have actually done together (decisions, deliverables, things discovered/broken/fixed). Read at least the most recent 5–10 entries. This exists because memory alone wasn't keeping Big C oriented session-to-session — Big C kept re-deriving (or mis-deriving) things Big D had already explained. **Append a new entry to it** at the end of any session where something durable happened (a decision, a build, a discovery, a fix) — follow the format documented at the top of that file.

For live pipeline state (pending approvals, slot status, blockers), pull current data directly from `logs/` and state files (`watch-drive-state.json`, `post-state.json`, etc.) rather than relying on a cached doc — it's fresher and more reliable.

Do not ask questions answered in `BSV-Memory.md` or `BSV-BigC-Audit-Log.md`.
...
Note: `BSV-Session-Context.md` is generated by `update-handoff.js` but only ever lands in `~/tmp/bsv-handoff/` and Drive's `Handoff/` folder — never the project repo. It is not part of this protocol; do not flag it as missing.
