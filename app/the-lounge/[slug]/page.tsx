...
import { getAllLoungePosts, getLoungePostBySlug } from '@/lib/lounge'
...
export async function generateStaticParams() {
  return getAllLoungePosts().map((post) => ({ slug: post.slug }))
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const post = getLoungePostBySlug(params.slug)
  if (!post) return {}
  return {
    title: post.title,
...
const mdxComponents = {
  ...
  em: (props: React.HTMLAttributes<HTMLElement>) => (
    <em className="text-bsv-muted italic" {...props} />
  ),
  ...
  a: (props: React.AnchorHTMLAttributes<HTMLAnchorElement>) => (
    <a className="text-bsv-amber hover:text-orange-400 underline underline-offset-2 transition-colors" {...props} />
  ),
...
export default function LoungePostPage({ params }: Props) {
  ...
  const postUrl = `${siteUrl}/the-lounge/${post.slug}`
        ...
        <section className="py-20 bg-bsv-card border-b border-white/5">
          <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex flex-wrap items-center gap-3 mb-4">
              <span className="text-bsv-orange text-xs font-heading tracking-widest border border-bsv-orange/30 px-2 py-0.5">
                CHAPTER {post.chapter}
              </span>
              <span className="text-bsv-orange text-xs font-heading tracking-widest border border-bsv-orange/30 px-2 py-0.5">
                {post.type === 'hub' ? 'HUB' : 'DEEP DIVE'}
              </span>
              {post.tags?.map((tag) => (
                <span
                  key={tag}
                  className="text-bsv-muted text-xs font-heading tracking-widest border border-white/10 px-2 py-0.5"
                >
                  {tag.toUpperCase()}
                </span>
              ))}
            ...
            <h1 className="font-heading text-4xl sm:text-5xl text-bsv-white tracking-wide leading-tight mb-6">
              {post.title}
            </h1>
