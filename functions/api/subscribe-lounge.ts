export async function onRequestPost(context: any) {
  ...
  const listId = context.env.KLAVIYO_LOUNGE_LIST_ID
  const headers = {
  ...
  if (listId) {
    ...
    if (!listRes.ok && listRes.status !== 204) {
      return new Response(JSON.stringify({ error: 'Failed to add to Lounge list' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      })
    }
