export async function onRequestPost(context: any) {
  ...
  const listId = context.env.KLAVIYO_KICKOFF_LIST_ID
  const headers = {
  ...
  // Step 1: create profile with segment property
  ...
  if (listId) {
    ...
    if (!listRes.ok && listRes.status !== 204) {
      return new Response(JSON.stringify({ error: 'Failed to add to Kick Off list' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      })
    }
