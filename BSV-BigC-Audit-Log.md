...
**Not fixed — bug #1, mon-pm-flow's missing media, needs Big D's call before touching anything.** The caption-only leftover can't post as-is under this pipeline's media+caption model. `clear_drive_slot('mon-pm-flow')` would remove it from Drive + local mirror + state cleanly, but that's a deletion, and Hard Rules require Big D's explicit go-ahead for that every time — asked, not executed.
