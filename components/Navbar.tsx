    ...
    <>
      <nav className="fixed top-0 left-0 right-0 z-50 bg-bsv-bg/95 backdrop-blur border-b border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            {/* Logo */}
            <Link href="/" className="font-heading text-2xl tracking-wider text-bsv-orange">
              BIG SOLE VIBES
            </Link>
            ...
            <div className="hidden md:flex items-center gap-8">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="text-sm font-medium text-bsv-muted hover:text-bsv-white transition-colors"
                >
                  {link.label}
                </Link>
              ))}
            ...
            {/* Hamburger — mobile */}
            <button
              className="md:hidden text-bsv-white p-2 z-50"
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 h-16 border-b border-white/10">
          <span className="font-heading text-xl text-bsv-orange tracking-wider">MENU</span>
          <button
            onClick={() => setMenuOpen(false)}
            aria-label="Close menu"
            className="text-bsv-muted hover:text-bsv-white transition-colors"
          >
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex flex-col px-6 py-8 gap-2 flex-1">
          {navLinks.map((link) => (
            <Link
              ...
              href={link.href}
              onClick={() => setMenuOpen(false)}
              className="font-heading text-3xl tracking-wide text-bsv-white hover:text-bsv-orange transition-colors py-2 border-b border-white/5"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="px-6 pb-10">
          <p className="text-bsv-muted text-xs font-heading tracking-widest mb-4">FOLLOW US</p>
