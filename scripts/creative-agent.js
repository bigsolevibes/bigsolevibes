...
;(async function run() {
  ...
  // Visual approach branch — added 2026-07-02. media-director.js assigns
  // visualApproach per slot (see pickVisualApproach there for why: photorealistic
  // human-in-scene generation was unreliable — framing, product legibility, setting
  // drift, verified via repeated live tests — while still-life and illustration are
  // proven on the actual live site (public/brand/bsv-hero-foundation.png,
  // public/crawl/*.jpg) and already sanctioned by BSV's own visual philosophy
  // ("illustration mix welcome"). This branches the instruction so creative-agent
  // asks Imagen for what it's actually good at, instead of the hardest case every time.
  ...
  const imageBriefInstruction = editionVignette
    ...
    ? `Gemini Imagen 4. Square 1:1. No text, no logos, no human figure required. Flat 2D cutout collage animation, Monty Python Flying Circus style (Terry Gilliam) — the same proven style used in public/crawl/*.jpg. Stiff paper-cutout shapes, flat blocks of color, visible torn-paper edges, deadpan absurdist tone, no shading or photographic depth. Warm amber (#C17D2E) and deep navy (#0D1B2A) anchor the palette.${product ? ` ${product['Product Name']} appears as a flat cutout element in the collage — its container shape and color should be recognizable even in this flat style.` : ''} Write ONE scene description, 3-5 sentences: what's depicted, the composition, the mood. The image poses a question, it does not answer one. REJECTED without appeal if: photorealistic rendering, multiple frames, any text or logo in the image.`
    : visualApproach === 'still-life'
    ? `Gemini Imagen 4. Square 1:1. No text, no logos, no human figure. A still-life composition in the style of public/brand/bsv-hero-foundation.png — objects arranged with intention, warm directional light, shallow depth of field, cinematic mood.${product ? ` ${product['Product Name']} is the central object — describe its actual container/shape/color in specific physical detail, in sharp focus, composed so the eye lands on it first.` : ' Build the composition around objects that evoke the theme below without needing a human subject.'} One or two other objects may share the frame to build a story (a glass, a pair of boots, a folded towel) but the named product stays the visual anchor. Warm amber (#C17D2E) and deep navy (#0D1B2A) palette. Write ONE scene description, 3-5 sentences. REJECTED without appeal if: a human figure appears, multiple frames, any text or logo in the image, the product missing or not the clear focal point.`
